...
void ImportAddress(const CLitecoinAddress& address, const string& strLabel);
void ImportScript(const CScript& script, const string& strLabel, bool isRedeemScript)
{
    ...
    if (isRedeemScript) {
        ...
        ImportAddress(CLitecoinAddress(CScriptID(script)), strLabel);
    } else {
...
UniValue importpubkey(const UniValue& params, bool fHelp)
{
    ...
    ImportAddress(CLitecoinAddress(pubKey.GetID()), strLabel);
...
UniValue importwallet(const UniValue& params, bool fHelp)
{
    ...
    while (file.good()) {
        ...
        if (pwalletMain->HaveKey(keyid)) {
            LogPrintf("Skipping import of %s (key already present)\n", CLitecoinAddress(keyid).ToString());
            continue;
        }
...
UniValue dumpprivkey(const UniValue& params, bool fHelp)
{
    ...
    return CLitecoinSecret(vchSecret).ToString();
}
...
UniValue dumpwallet(const UniValue& params, bool fHelp)
{
    ...
    for (std::vector<std::pair<int64_t, CKeyID> >::const_iterator it = vKeyBirth.begin(); it != vKeyBirth.end(); it++) {
        ...
        std::string strAddr = CLitecoinAddress(keyid).ToString();
        CKey key;
        if (pwalletMain->GetKey(keyid, key)) {
            file << strprintf("%s %s ", CLitecoinSecret(key).ToString(), strTime);
